  ...
  try {
    ...
    for (const [path, pathItem] of Object.entries(swaggerSchema.paths)) {
      ...
      for (const method of ["get", "post", "put", "patch", "delete"]) {
        ...
        const hasBody =
          ["post", "put", "patch"].includes(method) && operation.requestBody;
        ...
        if (!hasPathParams && !hasQueryParams && !hasBody) {
          ...
          const pathParamsStr = pathParams
          ...
          if (hasBody) {
            ...
            if (hasQueryParams) {
              const queryParamsType = operation.parameters?.some(
                (p) => p.in === "query",
              )
        ...
        const apiLineFormat = rmMethodPrefix
          ? apiLine.replace(`${groupName}_`, "")
          : apiLine;
